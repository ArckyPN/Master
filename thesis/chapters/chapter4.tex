...
To simplify the usage of these complex options I have created an helper script to easily use the Producer with a few pre-configured methods during development, details will follow in \Cref{sec:docu}.
...
The input of the live stream is configured using an subcommand of the Producer. I will give each type of input its own section.
...
The \textbf{local} command takes a local files as input. This is essentially the same as the remote command, the difference being that is uses an local path instead of a remote URL. The same restricts apply.

The path is supplied with a positional argument. However, it has a second option. This being whether or not to create a live stream. For example when using a video file as input, this video has a finite runtime. Setting this flag will make FFmpeg endlessly loop through the file to create an live stream.
...
The only required options is the resolution of the video data, given with the height and width of the video in number of pixels.
...
In addition to the general options the \texttt{c2patool} uses a few commands for additional configuration. As previously mentioned I have added the "live" command to the tool. This command turns the \texttt{c2patool} into an HTTP server to receive the live stream from the Producer. The target URL is the ingest URL of the CDN.
